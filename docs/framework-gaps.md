...
| 起牌 | `core/src/setup.ts`、`core/src/state.ts` | ✅ 掷点定先手（D6 平局重掷）→ 换牌（换 N 补 N，每方一次）→ 后手补传国玉玺 |
...
| ~~酒令~~ | ~~`core/src/jiuling.ts`~~ | ❌ **已整套撤除**（ADR-048：设计者确认非其设计，原为 GDD 11 §3 草案） |
...
| # | 缺口 | 影响 | 现状 |
|---|---|---|---|
| ~~G-1~~ | ~~构筑界面~~ | ✅ **已完成**（`prototype/setup.js`）：阵营选择 → 卡池筛选（费用/类型）→ 逐张加卡 → 实时曲线 + 合法性校验 + 自动组卡 | — |
| ~~G-2~~ | ~~换牌交互~~ | ✅ **已完成**：起手展示 + 点选要换的牌 + 换 N 补 N；敌方 AI 也会换掉最贵的 2 张 | — |
| G-4b | **事件日志** | ✅ **已完成**：右侧日志把 30 种事件翻成中文（含伤害数值、状态名、进化、粮尽、拼点等），做卡牌测试时能直接看到"这张卡到底做了什么" | — |
...
### 2.2bis 本轮新发现的分类/文案问题（见 §5 审计）

| # | 问题 | 说明 |
|---|---|---|
| G-19 | **曹操「护驾」分类与实现双重问题** | 文案是「**每回合开始时**，可选择护驾」，属于回合开始的抉择，数据却归成随时可点的 `active`；且文案有**两种模式**（①伤害由指定己方人物承受 ②指定敌方一人物+己方一血+抽1张+伤害−1），数据只实现了 ① |
| G-20 | **简雍「劝降」目标范围偏大** | 文案是「对指定**等于自己统帅**的卡牌」（简雍 3 费 → cost == 3），数据是 `cost_max: 3`（≤3），能劝降比文案更便宜的卡。DSL 缺 `cost_eq` |
| G-21 | **`frequency` 表达不了「每回合 N 次」** | 只有 `once_per_turn / unlimited / once` 三个值。华佗原文若真是「每回合 2 次」就装不下（现已按 1 次裁定，但同类卡以后会撞上） |
| G-22 | **`remove_status` / `move` / `random_pick` 未实现** | 与 G-6 同一项，此处仅补记：`remove_status` 已计价，华佗「青囊」的"清除负面效果"目前是空转 |
